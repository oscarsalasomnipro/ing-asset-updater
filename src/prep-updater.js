...
async function main() {
    ...
    try {
        ...
        let assetsToUpdate = await localAssets.getAssetsToUpdate(configFile?.local?.assetsFile?.path,
            configFile?.local?.assetsFile?.metadataMapping, {
            aemUsername: configFile?.aem?.username,
            aemPassword: configFile?.aem?.password,
